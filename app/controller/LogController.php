    ...
    public function __construct(Setup $setup)
    {
        ...
        if (!empty($history)) {
            ...
            foreach ($entries as $revision) {
                ...
                if ($dosearch) {
                    ...
                    if (empty($fromRev) || $fromRev > $thisrev) {
                        // Turn all the HTML entities into real characters. Make sure that each word in the search in also in the log
                ...
                if ($match) {
                    ...
                    if ($showchanges) {
                        ...
                        $listvar['revadded']    = (isset($modpaths['A'])) ? implode(WebSvnCons::BR, escape($modpaths['A'])) : '';
                        $listvar['revdeleted']  = (isset($modpaths['D'])) ? implode(WebSvnCons::BR, escape($modpaths['D'])) : '';
                        $listvar['revmodified'] = (isset($modpaths['M'])) ? implode(WebSvnCons::BR, escape($modpaths['M'])) : '';
            ...
            if ($pages > 1) {
                ...
                if ($page > 1) {
                    $setup->vars['pagelinks'] .= '<a href="'.$logurl;
                    $setup->vars['pagelinks'] .= (!$setup->peg && $frev && $prev != 1 ? WebSvnCons::ANDAMP.'peg='.$frev : '');
                    $setup->vars['pagelinks'] .= WebSvnCons::ANDAMP.'page='.$prev.'">&larr;';
                    $setup->vars['pagelinks'] .= $setup->lang['PREV'].'</a>';
                } else {
                ...
                for ($p = 1; $p <= $pages; $p++) {
                    if ($p != $page) {
                        $setup->vars['pagelinks'] .= '<a href="'.$logurl;
                        $setup->vars['pagelinks'] .= (!$setup->peg && $frev && $p != 1 ? WebSvnCons::ANDAMP.'peg='.$frev : '');
                        $setup->vars['pagelinks'] .= WebSvnCons::ANDAMP.'page='.$p.'">'.$p.'</a>';
                    } else {
                ...
                if ($page < $pages) {
                    $setup->vars['pagelinks'] .= '<a href="'.$logurl;
                    $setup->vars['pagelinks'] .= (!$setup->peg && $frev ? WebSvnCons::ANDAMP.'peg='.$frev : '');
                    $setup->vars['pagelinks'] .= WebSvnCons::ANDAMP.'page='.$next.'">';
                    $setup->vars['pagelinks'] .= $setup->lang['NEXT'].'&rarr;</a>';
                } else {
