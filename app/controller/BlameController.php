                    ...
                    if ($blame = fopen($tbname, 'r')) {
                        ...
                        while (!feof($blame) && !feof($file)) {
                            ...
                            if (!empty($blameline)) {
                                ...
                                $setup->listing[$index]['lineno'] = $index + 1;
                                
                                if ($last_rev !== $revision) {
                                    
                                    $url = $setup->config->getURL($setup->rep, $setup->path, 'blame');
                                    $setup->listing[$index]['revision'] = '<a id="l'.$index.'-rev" class="blame-revision" href="'.$url.'rev='.$revision.WebSvnCons::ANDAMP.'peg='.$setup->rev.'">'.$revision.'</a>';
                                    $seen_rev[$revision] = 1;
                                    $row_class = ($row_class == 'light') ? 'dark' : 'light';
                                    $setup->listing[$index]['author'] = $author;
                                    
                                } else {
                                    
                                    $setup->listing[$index]['revision'] = '';
                                    $setup->listing[$index]['author'] = '';
                                    
                                }//end if
                                
                                $setup->listing[$index]['row_class'] = $row_class;
                                $last_rev = $revision;
                                ...
                                $setup->listing[$index]['line'] = ($empty) ? '&nbsp;' : $setup->utils->wrapInCodeTagIfNecessary($line, $setup->config->getUseGeshi());
                                $index++;
